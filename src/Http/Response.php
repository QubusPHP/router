...
class Response
{
     ...
     * @param null|int               $options JSON options Bitmask consisting of JSON_HEX_QUOT, JSON_HEX_TAG, JSON_HEX_AMP, JSON_HEX_APOS, JSON_NUMERIC_CHECK, JSON_PRETTY_PRINT, JSON_UNESCAPED_SLASHES, JSON_FORCE_OBJECT, JSON_PRESERVE_ZERO_FRACTION, JSON_UNESCAPED_UNICODE, JSON_PARTIAL_OUTPUT_ON_ERROR.
    ...
    public function json($value, ?int $options = null, int $dept = 512): void
    {
        if ($value instanceof JsonSerializable === false && is_array($value) === false) {
            throw new TypeException('Invalid type for parameter "value". Must be of type array or object implementing the \JsonSerializable interface.');
        }
